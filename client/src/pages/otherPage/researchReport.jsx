...
const RealEstateResearchReports = () => {
  ...
  return (
          <>
          <div className='w-[100%] h-19 bg-indigo-600'></div>
    <div className="min-h-screen bg-white py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-extrabold text-indigo-600 sm:text-2xl lg:text-5xl">
        ...
        <div className="mb-8 bg-white p-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="w-full md:w-1/2">
              <label htmlFor="search" className="block text-sm font-medium text-indigo-600 mb-1">
                Search Reports
              </label>
              <div className="relative rounded-md shadow-sm">
                <input
                  type="text"
                  id="search"
                  className="block w-full pl-4 pr-10 py-2 border-indigo-600 outline-none rounded-md  border" 
            ...
            <div className="w-full md:w-1/2">
              <label htmlFor="category" className="block text-sm font-medium text-indigo-600  mb-1">
                Filter by Category
              </label>
              <select
                id="category"
                className="block w-full pl-3 pr-10 py-2 text-base border border-indigo-600 outline-none rounded-md"
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 ">
          {filteredReports.length > 0 ? (
            filteredReports.map(report => (
              <div key={report.id} className="bg-white overflow-hidden shadow rounded-lg hover:shadow-lg transition-shadow border border-indigo-600">
                <div className="p-6">
                  <h3 className="text-lg font-semibold text-indigo-600 mb-2">{report.title}</h3>
                  <p className="text-sm text-indigo-600 mb-4">{report.description}</p>
                  <div className="flex justify-between items-center text-sm text-indigo-600">
                    <span>{report.date}</span>
                    <button
                      className="text-indigo-600 cursor-pointer hover:text-indigo-600 font-medium"
            ...
            <p className="text-center text-indigo-600 col-span-full">No reports found.</p>
          )}
        </div>
      </div>
    </div>
