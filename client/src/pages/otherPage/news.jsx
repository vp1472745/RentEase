...
<>
    ...
    <div className="min-h-screen bg-gray-50 px-6 py-10">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-bold text-center mb-6 text-indigo-600">Real Estate News & Updates</h1>

        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
          <input
            type="text"
            placeholder="Search news..."
            className="w-full md:w-1/2 border border-indigo-600 rounded-md px-4 py-2 outline-none cursor-pointer"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />

          <select
            className="w-full md:w-1/2 border border-indigo-600 rounded-md px-4 py-2 cursor-pointer"
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredNews.length > 0 ? (
            filteredNews.map(news => (
              <div key={news.id} className="bg-white p-5 rounded-lg shadow hover:shadow-md transition duration-200 border-2 border-indigo-600">
                <h2 className="text-xl font-semibold text-indigo-600">{news.title}</h2>
                <p className="text-sm  mb-2 text-indigo-600">{news.date} | {news.category}</p>
                <p className="text-indigo-600 mb-4">{news.description}</p>
                <a href={news.source} target="_blank" rel="noopener noreferrer" className="text-indigo-600 ">
                  Read More →
                </a>
              </div>
            ))
          ) : (
            <p className="text-center text-indigo-600 col-span-full">No news found matching your criteria.</p>
          )}
        </div>
      </div>
    </div>
